...
function initialize(){
  ...
  safeAdd("threeEditImport", "click", () => document.getElementById("threeEditImportFile")?.click());
  ...
  safeAdd("threeLabelImport", "click", () => document.getElementById("threeLabelImportFile")?.click());
  ...
  safeAdd("threeDestinationImport", "click", () => document.getElementById("threeDestinationImportFile")?.click());
...
function renderRoute(route){
  ...
  if(!certified){
    ...
    if (wt) wt.disabled=true;
    if (routeStatus) routeStatus.textContent="3D route hidden — trace and approve the pedestrian network first";
    return;
  }
  if(routeToggle) routeToggle.disabled=false;
  if(routeStatus) routeStatus.textContent="Approved pedestrian route loaded";
  const points=routePositions(route);
  const walkButton=document.getElementById("threeWalkToggle");
  if(points.length<2){
    if(walkButton) walkButton.disabled=true;
...
function toggleRouteEditor(){
  ...
  if(hint) hint.textContent=editorActive
...
function updateEditorControls(){
  ...
  const statusEl = document.getElementById("threeEditStatus");
  if(statusEl) statusEl.textContent=editorActive
...
function saveEditorDraft(){
  persistEditorDraft();
  const st = document.getElementById("threeEditStatus");
  if(st) st.textContent=`Network saved locally • ${editorNodes.length} nodes / ${editorEdges.length} walkways`;
}
...
async function importEditorDraft(event){
  ...
  try{
    ...
    rebuildEditorVisuals();
    const st = document.getElementById("threeEditStatus");
    if (st) st.textContent=`Imported ${file.name} • ${editorNodes.length} nodes / ${editorEdges.length} walkways`;
  }catch(error){
...
function startLabelPlacement(){
  ...
  const st2 = document.getElementById("threeLabelStatus");
  if (st2) st2.textContent="Click the exact feature in the 3D model";
  const hint = document.querySelector(".three-hint");
  if (hint) hint.textContent="Click the exact label anchor location";
}
...
function cancelLabelPlacement(){
  ...
  const st = document.getElementById("threeLabelStatus");
  if (st) st.textContent="Label placement cancelled";
  const hint = document.querySelector(".three-hint");
  if (hint) hint.textContent="Drag to orbit • Scroll to zoom • Right-drag to pan";
}
...
function placeLabelOverride(local){
  ...
  const st = document.getElementById("threeLabelStatus");
  if(st) st.textContent=`Placed ${record.name} • auto-saved locally`;
  const hint = document.querySelector(".three-hint");
  if(hint) hint.textContent="Drag to orbit • Scroll to zoom • Right-drag to pan";
}
...
function saveLabelOverrides(){
  persistLabelOverrides();
  const st = document.getElementById("threeLabelStatus");
  if(st) st.textContent=`Saved ${labelOverrides.length} label override${labelOverrides.length===1?"":"s"} locally`;
}
...
function removeLabelOverride(){
  ...
  persistLabelOverrides();
  const st = document.getElementById("threeLabelStatus");
  if(st) st.textContent="Override removed; recovered label restored • auto-saved";
}
...
async function importLabelOverrides(event){
  ...
  try{
    ...
    persistLabelOverrides();
    const st = document.getElementById("threeLabelStatus");
    if (st) st.textContent=`Imported ${file.name} • ${labelOverrides.length} labels • auto-saved`;
  }catch(error){
...
function selectDestinationSource(){
  ...
  const st = document.getElementById("threeDestinationStatus");
...
function startDestinationPlacement(){
  ...
  if(!sourceId||!name){
    const st = document.getElementById("threeDestinationStatus");
    if (st) st.textContent="Choose a mapped label and destination name first";
    return;
  }
  if(editorActive)toggleRouteEditor();
  if(labelPlacementActive)cancelLabelPlacement();
  ...
  if (pl) pl.classList.add("active");
  const st2 = document.getElementById("threeDestinationStatus");
  if(st2) st2.textContent="Click the pedestrian entrance or arrival point";
  const hint = document.querySelector(".three-hint");
  if(hint) hint.textContent="Click the exact pedestrian arrival point for this destination";
}
...
function cancelDestinationPlacement(){
  ...
  if(pl) pl.classList.remove("active");
  const st = document.getElementById("threeDestinationStatus");
  if (st) st.textContent="Destination placement cancelled";
  const hint = document.querySelector(".three-hint");
  if(hint) hint.textContent="Drag to orbit • Scroll to zoom • Right-drag to pan";
}
...
async function importDestinationDrafts(event){
  ...
  try{
    ...
    populateDestinationSources();
    const st = document.getElementById("threeDestinationStatus");
    if(st) st.textContent=`Imported ${file.name} • ${destinationDrafts.length} destination anchors • auto-saved`;
  }catch(error){
